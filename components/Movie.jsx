...
const Pic = styled.div`
  ...
  ${tw` w-11/12 mx-auto bg-cover bg-center rounded-lg cursor-pointer`}
...
const Group = styled.div`
  height: 3rem;
  width: 100%;
  ${tw` w-11/12 flex justify-between items-center gap-1 `}
`;
...
const Movie = ({ image, url, title, vote, id }) => {
  return (
    <Container>
      <Link href={`/movie/${id}`} style={{ width: "100%" }}>
        <Pic image={image} url={url}></Pic>
      </Link>
      <Group>
        <Link href={`/movie/${id}`} style={{ width: "60%" }}>
          <Title>{title}</Title>
        </Link>
        <Rating>
